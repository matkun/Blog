	...
	$('input.saveFileButton').click(function () {
		...
		$.ajax({
			...
			success: function (response) {
				$('span#Message').addClass("show").text("Successfully saved changes to the file " + targetFilename + ".");
			...
			error: function (response) {
				$('span#Message').addClass("show").text("Failed! Nothing saved: " + response.status + " " + response.statusText);
			}
		});
	});
});
